 ...
 * Incorpora doble verificación en:
 * 1. Eliminación de usuarios (modal de confirmación)
 * 2. Guardado / Registro de usuarios (paso de confirmación previa)
...
export default function AltasRfidView({ showToast }) {
  ...
  const usuariosFiltrados = usuarios.filter((u) => {
    const matchesSearch =
      ...
      u.matricula.toLowerCase().includes(searchUser.toLowerCase()) ||
      u.correo.toLowerCase().includes(searchUser.toLowerCase()) ||
      u.uidRfid.toLowerCase().includes(searchUser.toLowerCase());

    const matchesRol = rolFilter === 'todos' ? true : u.rol === rolFilter;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-xs flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-96">
          <input
            type="text"
            placeholder="Buscar por nombre, matrícula, correo o UID..."
            value={searchUser}
            onChange={(e) => {
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="font-mono text-[11px] text-slate-500">ID: {u.matricula}</span>
                        </div>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-slate-600">
              Al confirmar, el usuario será retirado del padrón y su tarjeta RFID dejará de tener acceso autorizado en el ESP32.
            </p>
